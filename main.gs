...
function MessageArchive(){
  ...
  if (json!='None'){
    ...
    for(let i=0;i<json.length;i++){
      // メッセージを送った時の日本標準時、発言者氏名、メッセージを格納
      var jstTime=Utilities.formatDate(new Date(json[i].send_time*1000),"JST","yyyy/MM/dd HH:mm");
...
function sendMessage(_token){
  ...
  var strBody="[info][title] Message Archive Monthly-Report"+d+"[/title]運用開始からの保存状況は、以下の通りです。\n"+
              "・Hogeチャット："+num_message.toString()+"メッセージ（1万メッセージまで、あと"+(10000-num_message).toString()+"メッセージ）";
